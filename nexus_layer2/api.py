...
Uses unified NexusLLM (WatsonX/Gemini) and RTX GPU acceleration.
Endpoints: DPRs, graph, decay, counterfactuals, knowledge, risk report, query, analyze, gpu, llm.
"""
    ...
    if req.use_gemini:
        try:
            ...
            if llm.is_available:
                ...
                llm_answer = llm.generate(prompt, temperature=0.3, max_tokens=4000)
        except Exception as e:
            llm_answer = f"LLM unavailable: {str(e)[:100]}"

    return {
        "question": req.question,
        "results": results[:5],
        "llm_answer": llm_answer,
        "gemini_answer": llm_answer,  # backward compat
...
async def analyze_repository(req: AnalyzeRequest, background_tasks: BackgroundTasks):
    ...
    def run_analysis():
        try:
            ...
            from repo_analyzer import clone_repo, extract_dprs, build_nexus_data_from_extraction
            
            # Stage 1: Clone (with streamed progress)
            _set_analysis_state(stage="cloning", progress=5)
            repo_path = clone_repo(url, progress_callback=lambda **kw: _set_analysis_state(**kw))
            
            # Stage 2: Indexing
            _set_analysis_state(stage="indexing", progress=20)
            
            # Stage 3: Extraction
            _set_analysis_state(stage="extracting", progress=25)
            
            l1_data = extract_dprs(url, repo_path, window,
                                   progress_callback=lambda **kw: _set_analysis_state(**kw))
...
async def get_gpu_status():
    """Get GPU acceleration status (RTX vs Intel)."""
    try:
        from gpu_accelerator import gpu_status, gpu_benchmark
        status = gpu_status()
        return status
    except Exception as e:
        return {"error": str(e), "gpu_active": False}


@app.get("/api/llm")
async def get_llm_status():
    """Get LLM provider status (WatsonX/Gemini)."""
    try:
        from llm_provider import get_llm
        llm = get_llm()
        return llm.status()
    except Exception as e:
        return {"error": str(e), "active_provider": "none"}
