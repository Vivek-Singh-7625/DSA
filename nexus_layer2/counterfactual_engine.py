...
Uses WatsonX/Gemini via unified NexusLLM + graph traversal.
"""
...
def get_nexus_llm():
    ...
    try:
        ...
        print(f"[!] LLM provider unavailable: {e}")
        return None
...
def simulate_counterfactual(llm, scenario, dpr, downstream_dprs, all_dprs):
    """Use WatsonX/Gemini to trace ripple effects of an alternative decision."""
    downstream_desc = ""
...
{{
  ...
  "modern_relevance": "Is this alternative worth revisiting today? Why or why not?"
    ...
    result = llm.generate_json(prompt, temperature=0.3, max_tokens=4000)
    if result:
        return result

    return {
        "verdict": "tradeoff", "confidence": 0.5,
        "broken_assumptions": [], "unnecessary_workarounds": [],
        "new_problems": ["Analysis unavailable"],
        "affected_dprs": [],
...
def run_counterfactual_engine():
    ...
    llm = get_nexus_llm()
    if not llm:
        print("[!] LLM unavailable — counterfactuals will use placeholder data")
    else:
    ...
    for scenario in SCENARIOS:
        ...
        if llm:
            result = simulate_counterfactual(llm, scenario, dpr, downstream, dprs)
